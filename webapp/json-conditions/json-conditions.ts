...
// Taken from https://github.com/raisely/json-conditions/blob/master/index.js.
// Modified to be loadable here.
 ...
 */
export function checkConditions(settings, reference) {
  ...
  const results = settings.rules.map((rule, index) => {
    ...
    let value = reference[rule.property];
    if (rule.property.includes('[]')) {
      let [topPath, nestedPath] = rule.property.split('[]');
      nestedPath = nestedPath.substring(1);
      value = reference[topPath].map((item) => (nestedPath ? item[nestedPath] : item));
    }
    ...
    switch (rule.op) {
      ...
      case 'startsWith':
        ...
        break;
      case 'endsWith':
        result = `${value}`.endsWith(targetValue);
        break;
      case 'contains':
        result = `${value}`.includes(targetValue);
        break;
      case 'present':
        result = !!value;
        break;
